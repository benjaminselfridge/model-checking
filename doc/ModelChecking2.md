...
`(guard, action, loc)`. If `guard` is true of the current global state,
then modify the state by performing `action` and go to location `loc`.\"
When a guard is satisified in a given state, the corresponding
transition is said to be *enabled*. When multiple transitions are
