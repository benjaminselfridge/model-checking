...
Our language will be called `MIG`, which stands for `Modify`/`IfGoto`. In `MIG`,
a program is a sequence of statements. There are two kinds of statements:

  1. `Modify`: modify the global variable environment (e.g. assign a variable to
     a value)
...
A statement that modifies the global variable environment is represented as an
*effect*, which is a function taking the old environment to a new one:
...
A statement in MIG either modifies the current environment, or conditionally
...
environment, thus modifying it, and then go to the next line in the program. To
execute an `IfGoto` statement, we first test the `Predicate` against the
current environment: if the predicate evaluates to true, then we go to the
`LineNumber` indicated; if it is not true, then we go to the next line in the
program.

We'll also need an unconditional `goto` statement. We'll define it as `IfGoto
true`, where `true :: Predicate a` (defined in the previous post) is the
function that always returns `True`:

> goto :: LineNumber -> Stmt var val
> goto lineNum = IfGoto true lineNum

Another occasionally useful statement is `noop`, which is implemented as a
`Modify` that doesn't actually do anything:
...
The left-hand side of the `=` is a variable, and the right-hand side is an
*expression* that can be evaluated, given an environment that has a definition
for the variable `y`. In `MIG`, we could write the corresponding `Modify`
statement like so:
...
The function we passed to `Modify` took the current environment and modified it
by looking up the value of `Y`, adding `4` to it, and setting `X` equal to the
result. This is okay, but it would be much nicer to write something that looked
more like the corresponding C statement.

To accomplish this, we'll define an *assignment* operator that works on single
...
will mean "evaluate the expression `e` and assign the result to the variable
`x`". A simple way to represent an *expression* is as a function from the
...
This allows us to create an `Effect`, which is a function, without needing to
use an explicit lambda expression that calls `Map.insert`. Now, we can define
the `modify_stmt` statement a bit more nicely:
...
This is better. However, the `Expr` we are binding `X` to is still defined in
terms of a lambda expression and an explicit `Map.!` operator. We can do a bit
better still by defining some more functions to build `Expr`s more cleanly.

If `x :: var` is a variable, we can create a corresponding expression for `x`.
In our representation, the *expression* for `x` will be a function that simply
looks up `x` in the environment and returns its value.

> var :: Ord var => var -> Expr var val
> var x env = env Map.! x

If `c :: val` is a constant value, we can create a corresponding expression for
`c`. In our representation, the *expression* for `c` will be a function that
ignores the current environment and blindly returns the value `c`.

> val :: val -> Expr var val
> val c _ = c

If `val` is a numeric type, we can lift the usual numeric operators to
expressions. This can actually be accomplished by providing an orphan `Num`
...
Since an `Expr var val` is just a function `Env var val -> val`, this means that
if `val` has a `Num` instance, we have a corresponding instance for `Expr var
val` that behaves in the way we'd expect.
...
performing `a`, then performing `b`. This is useful when we wish to update two
variables atomically, i.e. perform both updates in a single step of the program.

The `IfGoto` statement
--

In this section, we'll define a few helper functions to help us write `IfGoto`
statements in a readable way. `IfGoto` takes an *environment predicate* and a
...
Therefore, a `Predicate (Env var val)` is a function `Env var val -> Bool`. If
this function evaluates to `True` in the current environment, the line number
should change to the value specified by the second argument of `IfGoto`, the
`LineNumber`.
...
Ideally the programmer doesn't use explicit `goto`s, but in `MIG` it will be the
only option for affecting control flow in our programs. Let's assume `label`
refers to a specific line number, namely line 17 of the program. Then we can
write the corresponding `IfGoto` statement like so:
...
The function we passed to `IfGoto` took the current environment, looked up the
values of `X` and `Y`, and tested whether the value of `X` was equal to `1` plus
...
`pnot`, and `.->` as defined in the previous post; these enable us to "build up"
larger predicates out of smaller ones.

Now, we can implement the C statement `if (x == 1 + y) goto label;` in a much
nicer way:
...
factorial. We won't do any model checking of this program; this is merely to
give you (the reader) a concrete sense of how programs can be written in it.
...
into a MIG program. The program has three variables: `n`, 'i', and `res`:
...
In this section, we will examine the problem of model checking multiple programs
running in parallel, which all have access to the same global variable
environment. We will use MIG express each individual program, and then we will
define a function to convert a parallel program into a transition system.

A *parallel program* is just a `Vector` of sequential programs with a shared
environment:
...
To "run" a parallel program, we first start each
process at line 0. Then, we arbitrarily pick which process will execute next,
and we execute that process's current statement, updating the global variable
environment and/or that process's current line number. This type of parallel
execution model is called *interleaving*, because each process advances
independently of the other, and the processes can advance in any order.

Example: Peterson's mutual exclusion algorithm
--

Consider two processes, P0 and P1, which are running corresponding programs.
Suppose that each program has a *critical section*, and if P0 and P1 are every
both in their critical sections, bad things can happen. The property that both
processes cannot execute their critical sections simultaneously is called
*mutual exclusion*.

Peterson's algorithm is an elegant way to ensure mutual exclusion. It uses three
variables: `Turn`, `Wait0`, and `Wait1`.

> data PeteVar = Turn | Wait0 | Wait1 deriving (Show, Eq, Ord)

Intuitively, the meaning of each variable is as follows:

* `Turn` is `False` it if is `P0`'s turn to enter its critical section, and
  `True` if it is `P1`s turn.
* `Wait0` is `True` if `P0` wishes to enter its critical section or is currently
  in its critical section, and `False` otherwise.
* `Wait1` is `True` if `P1` wishes to enter its critical section or is currently
  in its critical section, and `False` otherwise.

We implement Peterson's algorithm in MIG as a parallel program with two
processes. Each process is in an infinite loop, continually attempting to enter
its own critical section. When a process wants to enter its critical section, it
first sets its own `Wait` variable to `True`, and then it (somewhat
counterintuitively) signals that it is the *other* process's turn. Then, before
it enters its own critical section, it busy-waits until either the other process
is not waiting, or its own turn has arrived. Finally, when the critical section
is complete, the process sets its `Wait` variable to `False`, indicating it has
exited its critical section, and the other process is free to enter theirs. The
process then loops back to the beginning, and again attempts to enter its
critical section.
...
It is possible to reason through why this solution works, and even to write a
formal proof that both processes cannot be at line 3 simultaneously. However, by
converting this parallel program to a transition system, we can automatically
check that Peterson's algorithm ensures mutual exclusion. In the next section,
we discuss how to perform this conversion, and then apply it to this algorithm.

From parallel programs to transition systems
--

In order to model check a parallel program, we first convert such a program into
a transition system. The basic idea for the conversion will be that a state in
the transition system will be a pair `(Vector LineNumber, Env var val)`,
...
choice to the caller. The caller will provide a list of atomic propositional
variables, along with a function which maps each variable to a *state
predicate*, i.e. a `Predicate (ParProgState var val)`. Then, the label of each
state will be the set of variables whose corresponding predicate is true about
the current state.

The `action` type will just be a `(ProcId, LineNumber)` pair, corresponding to
"performing the statement at the given line of the given process." As in the
previous post, the `action` is just a name for each transition, and does not
have any semantic content whatsoever.

Let's define `parProgToTS`, which converts a parallel program to a transition
system:
...
considering. `aps` is the complete list of atomic propositional variables, and
`apToPred` maps each atomic propositional variable to its corresponding state
predicate. For each initial environment provided by the caller, there is a
corresponding initial state in the transition system with every process starting
at line 0:

>   { tsInitialStates = [(Vec.replicate (Vec.length parProg) 0, env) | env <- initialEnvs]

As for sequential programs, states are labeled by the atomic propositional
variables whose corresponding predicates they satisfy:

>   , tsLabel = \s -> [ p | p <- aps, s |= apToPred p ]

Each state has one outgoing transition for every process in the input program,
corresponding to executing the current line of that particular process.
...
The property we would like to hold about Peterson's algorithm is simple: that
both processes are never in their critical sections simultaneously. To ensure
that this is true, we use the following type for the atomic propositional
variables:

> data ProcAtLine = ProcAtLine ProcId LineNumber
>   deriving (Show, Eq, Ord)

`ProcAtLine procId lineNum` represents the predicate that "process `procId` is
currently at line `lineNum`." This is codified by the following function:
...
propositional variables. We need `ProcAtLine procId lineNum` for every valid
`(ProcId, LineNumber)` pair. We can collect all such pairs for an arbitrary
`ParProg` with the following function:
...
The critical section occurs at line 3 in both processes, so the property that
both processes are not simultaneously in their critical sections is stated as
follows:
...
assignment to the `Wait` variable occurs *before* the assignment to `Turn`. What
happens if we swap the two? Will our property still hold?
...
The naming of these programs probably tips you off about the outcome of our
little experiment.
...
it is P0's turn to enter the critical section. Then, P0 sets it to `True`,
indicating it is P1's turn. Then, at c, P0 sets `Wait0` to `True`,
indicating it wishes to enter. At line d, P0 enters its critical section even
though `Turn` is `True`, because `Wait1` is still `False`. Then, P1 is able to
enter *it's* critical section, because `Turn` is `True`.

We see that it *really matters* that the `Wait` variables are updated *before*
the `Turn` flag is flipped! Otherwise, the algorithm simply does not work.

What's next?
--

In this post, we showed how the basic idea of model checking can be applied to a
real-world program to prove something valuable and non-trivial. Peterson's
algorithm isn't *too* complicated, but it's not easy to see why it's correct at
first glance. We saw that by translating it to a transition system, we could
exhaustively explore the reachable state space, and found that it was impossible
to violated the desired invariant. When we modified the program in a subtle but
significant way, we discovered that the invariant failed.. Furthermore, the
model checking approach discovered a counterexample that helped us to understand
*why* it failed.
...
invariants; for instance, the property that a traffic light will be green
infinitely often, or that a yellow light always precedes a red light. In the
next post, we will explore a larger class of properties called *regular safety
properties*. We will show how to use *nondeterministic finite automata* to
express such properties, and how to check whether these properties hold.
