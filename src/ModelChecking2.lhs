...
To evaluate an expression, we just *apply* it (as a function) to an environment:
...
Each state `(lineNum, env) :: (LineNumber, Env var val)` as a single outgoing
transition.
...
deterministic sequential program for that matter. We're not going to be able to
prove anything in general about such a program, because in order to even convert
a program into a transition system, all of the variables must have concrete
values; therefore, "checking an invariant" of this system will be little more
than simulating the program for a particular input. For instance, suppose we
wanted to make sure that at each loop iteration, `Res * N! == n!`, where `n` is
the original input value and `N` is the current value of the `N` variable. We
could certainly do this:

This is nice, because it doesn't just check that result of the program is
correct; it actually shows that for input `4`, the values of `N` and `Res` are
consistent at every loop iteration. However, it's still deeply unsatisfying
because it doesn't say anything about arbitrary `N`.

This is less of an issue with parallel programs, which are inherently
nondeterministic; therefore, we'll focus on parallel programs and protocols for
the rest of this blog series. In the next section, we'll look at parallel
programs that operate on a shared state, and we'll model check a simple mutual
exclusion protocol.
