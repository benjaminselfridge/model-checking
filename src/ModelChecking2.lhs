...
which can only be followed when a particular condition holds, and which, when
followed, may change the global program state by modifying some subset of the
global variables. The idea is that when our program is in a location `loc` while
executing, it is allowed to follow any of `loc`'s outgoing transitions whose
guards are satisfied by the current state of the global variables.
...
Let's look at an example of an imperative program defined as a program graph,
and see how we can verify a simple property of this program. We will write a
program that simulates a soda machine. The machine works as follows: First, the
customer inserts a coin into the machine. Then, he selects "soda" or "beer", and
the soda machine either gives him a soda or gives him a beer. Additionally, a
maintenance technician can service the machine by collecting all of the coins,
and refilling the machine to its maximum capacity for both soda and beer.
...
coin, and the machine can either dispense a soda or a beer.

Now, let's create a program graph representing the soda machine. First we will
define our set of variables and locations:
